...
# HTTP/2 proxy in front of Puma in the container: serves the precompiled
# assets with gzip and far-future caching, which matters here because the
# Opal bundle is ~2.5MB uncompressed. bin/thrust and the Dockerfile CMD are
# generated by `rails new` expecting this gem.
